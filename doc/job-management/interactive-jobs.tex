...
\item Set up Conda environment by runnung \tool{setup\_conda.sh}
...
\textbf{Launching Jupyter Labs instance from \texttt{speed-submit}:}
\begin{enumerate}
...
% Jupyter Notebook in Python Virtual Environment
% ----------------------------------------------
...
\item Run \texttt{setup\_pyenv.sh} from \texttt{speed-hpc/src/jupyter/jupyterlabs-pyenv} on the compute node (not on \texttt{speed-submit})
\verb|./setup_pyenv.sh|

The script will:
\begin{itemize}
    \item Create a Jupyter directory
    \item Set environment variables such as \api{TMP}, \api{TMPDIR}, and \api{PIP\_CACHE\_DIR}
...
\item Run \texttt{start\_jupyterlab.sh} script from the submit node each time you need to launch juypterlab 
\verb|./start_jupyterlab.sh|

The script will:
\begin{itemize}
    \item Allocate resources for your job on a compute node
    \item Start Jupyter server by running \texttt{run\_jupyterlab.sh}
