...
\begin{figure}[htpb]
    \lstinputlisting[language=csh,frame=single,basicstyle=\ttfamily]{../src/tcsh.sh}
...
\begin{figure}[htpb]
    \lstinputlisting[language=csh,frame=single,basicstyle=\footnotesize\ttfamily]{../src/tmpdir.sh}
...
% TODO: fix RSA keys to be one of the EC ones as RSA is deprecated
\begin{itemize}
\item
\texttt{cd .ssh}
\item
...
\item
\texttt{cat id\_rsa.pub >> authorized\_keys} (if the \file{authorized\_keys}
file already exists) \emph{OR} \texttt{cat id\_rsa.pub > authorized\_keys} (if does not) 
\item
...
\begin{figure}[htpb]
    \lstinputlisting[language=csh,frame=single,basicstyle=\footnotesize\ttfamily]{../src/fluent.sh}
