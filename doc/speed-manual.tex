...
\xs{envsetup} is a good place to start. You can also use a job script
...
we recommend that your script use a shell from the \texttt{/encs/bin} directory. \par

To use the \texttt{tcsh} shell, start your script with: \verb| #!/encs/bin/tcsh| \par

For \texttt{bash}, start with: \verb| #!/encs/bin/bash| \par
...
32), be sure to delete or comment out the \verb| #$ -pe smp | parameter if it 
...
within the script just as it would be from the command line.\par
...
loop which iterates through a series of commands.\par
...
in your script by reviewing the software's documenation. Regardless of which software
...
to the value provided to the scheduler in the \option{-pe smp} option.\par 
...
than those to and from shared storage (which is provided over NFS). \par
...
for the super-ordinate array-job. To alter this behavior use the \option{-o} and \option{-e} option of \tool{qsub}. 

For more details about Array Job options, please review the manual pages for \option{qsub} by executing 
the following at the command line on speed-submit \tool{man qsub}.
