...
\item Does not run Docker. (Note: Speed does run Singularity and Docker containers can be converted to Singularity with a single command.)
\item Speed is not for jobs executed outside of the scheduler. (Jobs run outside of the scheduler will be killed and all data lost.)
\end{itemize}
...
After reviewing the "What Speed is" and "What Speed is Not" sections, request access to the ``Speed'' cluster by emailing: \href{mailto:rt-ex-hpc@encs.concordia.ca}{\texttt{rt-ex-hpc AT encs.concordia.ca}}.\\
\\Students must include the following in their message:
\begin{itemize} 
	\item ENCS username.
	\item Name of Supervisor or instructor.
...
Before Getting Started, please review the "What Speed is" and "What Speed is Not" sections.\\
...
From an ssh session on speed-submit, submit this job with \texttt{qsub ./tcsh.sh}. You will see, \texttt{"Your job 
