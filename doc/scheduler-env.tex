...
If any command is not found, e.g., (``command not found'') is returned,
you need to make sure your \api{\$PATH} includes \texttt{/local/bin}.
You can check your \api{\$PATH} by typing:
\texttt{echo \$PATH} at the prompt.\\
%
%source 
...
\begin{itemize}
    ...
    \verb|cp /winhome/<1st letter of $USER>/$USER/template.sh /speed-scratch/$USER/|
    \item From Linux drive to Speed:\\
    \verb|cp ~/template.sh /speed-scratch/$USER/|
\end{itemize}
